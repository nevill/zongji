...
var readRow = function(columns, parser) {
  var row = {};
...
WriteRows.prototype._fetchOneRow = function(parser) {
  ...
  return readRow(this.tableMap[this.tableId].columns, parser);
};
...
UpdateRows.prototype._fetchOneRow = function(parser) {
  ...
  var row = {};
  row.before = readRow(tableMap.columns, parser);

  // null bitmap 2
  parser.parseUnsignedNumber(
    Math.floor((this.numberOfColumns + 7) / 8));

  row.after = readRow(tableMap.columns, parser);
