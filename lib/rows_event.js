...
var readRow = function(tableMap, parser, nullBitmap) {
...
WriteRows.prototype._fetchOneRow = function(parser) {
  ...
  return readRow(this.tableMap[this.tableId], parser, nullBitmap);
};
...
UpdateRows.prototype._fetchOneRow = function(parser) {
  ...
  var nullBitmapBefore = parser.parseUnsignedNumber(
    Math.floor((this.numberOfColumns + 7) / 8));

  var row = {};
  row.before = readRow(tableMap, parser, nullBitmapBefore);

  var nullBitmapAfter = parser.parseUnsignedNumber(
    Math.floor((this.numberOfColumns + 7) / 8));

  row.after = readRow(tableMap, parser, nullBitmapAfter);
