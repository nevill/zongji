...
function RowsEvent(parser, options, zongji) {
  ...
  if(zongji._skipSchema(database, table)){
    ...
    if(columnsPresentBitmapSize > 4){
      throw new Error('too many columns (max 32) on: ' +
                      this.tableMap[this.tableId].parentSchema + '.' +
                      this.tableMap[this.tableId].tableName);
    }
    this.columnsPresentBitmap =
      parser.parseUnsignedNumber(columnsPresentBitmapSize);
...
var readRow = function(tableMap, parser) {
  ...
  var nullBitmap = parser.parseUnsignedNumber(
    Math.floor((tableMap.columns.length + 7) / 8));
