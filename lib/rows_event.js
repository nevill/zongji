...
const readRow = function(tableMap, parser, zongji) {
  let row = {}, column, columnSchema;
  const nullBitmapSize = Math.floor((tableMap.columns.length + 7) / 8);
  const nullBuffer = parser._buffer.slice(parser._offset,
                                        parser._offset + nullBitmapSize);
  let curNullByte, curBit;
  parser._offset += nullBitmapSize;

  for (let i = 0; i < tableMap.columns.length; i++) {
    curBit = i % 8;
