...
function parseBinaryBuffer(input, offset, parentValueOffset) {
  ...
  // No pointer if object is root
  var valueOffset = offset === 0 ? 0 : input.readUInt16LE(offset + 1) + parentValueOffset;

  console.log('offset', offset, valueOffset);
  var result = null;
  ...
  switch(jsonType) {
    ...
    case JSONB_TYPES.STRING:
      ...
      while(strLenSize === 0 || (curStrLenByte & 128) === 128) {
        ...
        console.log('lensize', strLenSize, 'curbyte', curStrLenByte);
        if(strLenSize === 1) {
    ...
    case JSONB_TYPES.SMALL_OBJECT:
      result = {};
      console.log('is small obj', valueOffset);
    ...
    case JSONB_TYPES.SMALL_ARRAY:
      ...
      while(pointerPos < memberCount) {
        memberValueOffset = 5 + (pointerPos * 3);
        result.push(parseBinaryBuffer(input, memberValueOffset));
        pointerPos++;
      }
      break;
    default:
