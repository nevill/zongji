...
var readTemporalFraction = function(parser, columnSchema) {
  ...
  return { value: fraction, precision: fractionPrecision };
};
  ...
  switch (column.type) {
    ...
    case MysqlTypes.DATETIME2:
      ...
      if(fraction !== false){
        ...
        var fractionStr = zeroPad(fraction.value, fraction.precision);
        if(fractionStr.length > 3){
          fractionStr = fractionStr.substr(0, 3);
        }else if(fractionStr.length === 2){
          fractionStr += '0';
        }else if(fractionStr.length === 1){
          fractionStr += '00';
        }
        result.setMilliseconds(parseInt(fractionStr, 10));
      }
      break;
