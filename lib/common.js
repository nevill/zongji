...
function parseBytesArray(parser, length) {
...
//                          (e.g. 'set(': 4, 'enum(': 5)
function parseSetEnumTypeDef(type, prefixLen){
...
function parseNewDecimal(parser, column) {
  // Constants of format
...
function convertToMysqlType (code) {
  return MysqlCodeTypes[code];
}
...
function readMysqlValue(parser, column, columnSchema, tableMap, emitter) {
  var result;
  // jshint indent: false
  switch (column.type) {
    case MysqlTypes.TINY:
    ...
    case MysqlTypes.DATETIME:
      ...
      if (utcOffset !== 0) {
        result = new Date(result.getTime() -  utcOffset);
      }
    ...
    default:
      result = undefined;
      emitter.emit('error',
        new Error('Unsupported type "' + convertToMysqlType(column.type) +
...
module.exports = {
  ...
  parseUInt48: parseUInt48,
  parseUInt24: parseUInt24,
  parseBytesArray: parseBytesArray,
  parseSetEnumTypeDef: parseSetEnumTypeDef,
  zeroPad: zeroPad,
  sliceBits: sliceBits,
  parseIEEE754Float: parseIEEE754Float,
  getUInt32Value: getUInt32Value,
  parseAnyInt: parseAnyInt,
  readInt24BE: readInt24BE,
  readIntBE: readIntBE,
  parseNewDecimal: parseNewDecimal,
  parseGeometryValue: parseGeometryValue,
  convertToMysqlType: convertToMysqlType,
  readTemporalFraction: readTemporalFraction,
  readMysqlValue: readMysqlValue,
