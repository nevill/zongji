...
exports.parseUInt40 = function(parser) {
  var low = parser.parseUnsignedNumber(4);
...
exports.readMysqlValue = function(parser, column, columnSchema) {
  ...
  switch (column.type) {
    ...
    case MysqlTypes.TIME2:
      ...
      var hasFraction =
        fractionPrecision !== undefined && fractionPrecision > 0;
      ...
      if(hasFraction){
        ...
        if(fractionPrecision % 2 !== 0) fraction /= 10;
      }
      ...
      if(hasFraction){
        result += '.' + zeroPad(fraction, fractionPrecision);
      }
      break;      
    ...
    case MysqlTypes.DATETIME2:
      var raw = exports.parseUInt40(parser);
      var date = Math.floor(raw / 1000000);
      var time = raw % 1000000;
      result = new Date(
        Math.floor(date / 10000),             // year
