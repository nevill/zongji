...
var getFractionString = function(fraction) {
  return fraction ? '.' + zeroPad(fraction.value, fraction.precision) : '';
};
...
var getDateString = function(year, month, date){
...
var getDate = function(dateStrings, // node-mysql dateStrings option
                       year,
                       month, // January === 1
                       date
                      )
...
var getDateTime = function(dateStrings, // node-mysql dateStrings option
                           year,
                           month,       // January === 1
                           date,
                           hour,
...
var getTimeStamp = function(dateStrings, // node-mysql dateStrings option
                           ...
                           )
{
  ...
  return getDateString(dateObject.getFullYear(),
         ...
         zeroPad(dateObject.getSeconds(), 2) +
         getFractionString(fraction);
};
  ...
  )
{
  ...
  switch (column.type) {
    ...
    case MysqlTypes.DATE:
      ...
      result = getDate(
        zongji.connectionConfig.dateStrings, // node-mysql dateStrings option
    ...
    case MysqlTypes.TIME2:
      ...
      if(fraction !== false){
        result += getFractionString(fraction);
      }
      break;
    ...
    case MysqlTypes.TIMESTAMP:
      var raw = parser.parseUnsignedNumber(4);
      result = getTimeStamp(zongji.connectionConfig.dateStrings, raw);
      break;
    case MysqlTypes.TIMESTAMP2:
      var raw = readIntBE(parser._buffer, parser._offset, 4);
      parser._offset += 4;
      var fraction = readTemporalFraction(parser, column.metadata.decimals);
      result = getTimeStamp(zongji.connectionConfig.dateStrings,
